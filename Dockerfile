...
COPY src ./latiendadelinfinito/src
COPY pom.xml ./latiendadelinfinito
RUN mvn -f ./latiendadelinfinito/pom.xml clean package -Dmaven.test.skip=true
...
#
FROM openjdk:8
COPY --from=build ./latiendadelinfinito/target/*.jar /usr/local/TDI/app.jar
EXPOSE 8080
